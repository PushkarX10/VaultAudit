...
import { OrdersWidget } from "../components/orders-widget";
  ...
  return (
    <Panel>
      <div className="flex items-center gap-3">
        ...
        <div className="text-[11px] tracking-[0.18em] uppercase text-white/40">{label}</div>
      </div>
      <div className="mt-4 text-white tracking-tight" style={{ fontSize: 22 }}>
        {value}
      </div>
      ...
      <PageHeader
        ...
        actions={
          <div className="inline-flex items-center gap-2 pl-2.5 pr-3.5 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.08]">
            ...
            <span className="text-[11px] text-white/70 tracking-wide">Encrypted vault · last synced 2m ago</span>
          </div>
        }
      />

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard icon={Wallet} label="Net Worth" value="$128,940.22" sub="+2.4% this month" tone="text-emerald-300/80" />
        <StatCard icon={TrendingUp} label="Portfolio" value="$47,320.10" sub="+$1,842 · 30d" tone="text-emerald-300/80" />
        <StatCard icon={Receipt} label="Spend · July" value="$1,904.55" sub="14 receipts scanned" tone="text-white/50" />
      </div>
