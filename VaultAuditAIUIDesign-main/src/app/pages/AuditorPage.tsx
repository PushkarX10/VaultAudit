...
import { Sparkles, Send, TrendingDown, TrendingUp, AlertTriangle, ChevronDown, Zap, Cpu } from "lucide-react";
...
function InsightCard({
  ...
  return (
    <div className={`rounded-2xl bg-gradient-to-b ${toneClass} border shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08)] overflow-hidden`}>
      <button
      ...
      {open && children && <div className="px-4 pb-4 border-t border-white/[0.06] pt-3">{children}</div>}
    </div>
  );
}
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <div className="inline-flex items-center gap-2 pl-2.5 pr-3.5 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.08]">
            ...
            <span className="text-[11px] text-white/70 tracking-wide">Model ready · 4.2 GB in RAM</span>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-1">
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  <div className="text-[11px] text-white/40 mb-1">VaultAudit · just now</div>
                  <div className="rounded-2xl rounded-tl-sm bg-white/[0.04] border border-white/[0.08] p-4 text-sm text-white/90">
                    I finished auditing your July transactions. Three anomalies stood out — click any card to
                    expand the analysis.
                  </div>
                </div>
              </div>
                  ...
                  <ResponsiveContainer>
                    <BarChart data={spendBars}>
                      <Bar dataKey="v" radius={[6, 6, 2, 2]}>
                        {spendBars.map((d) => (
                          <Cell key={d.c} fill={d.tone} fillOpacity={d.c === "Dining" ? 1 : 0.35} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
              ...
              >
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={m.l} className="rounded-lg bg-black/30 border border-white/10 p-2">
                      <div className="text-[10px] text-white/40 uppercase tracking-wider">{m.l}</div>
        ...
        <div className="space-y-4">
          <Panel>
            <div className="text-[11px] tracking-[0.18em] uppercase text-white/40">Quick Prompts</div>
          ...
          <Panel>
            <div className="flex items-center gap-2">
              ...
              <div className="text-[11px] tracking-[0.18em] uppercase text-white/40">Runtime</div>
            </div>
            <div className="mt-3 space-y-2 text-xs">
              {[
                ["Inference device", "Apple M3 · Neural Engine"],
